...
# Ключевые слова → показатель. Нужно, чтобы «свой чёрный лебедь» работал и без ключа LLM:
# грубо, зато честно помечено как шаблонный режим.
_KEYWORDS = [
    (("теплотрасс", "отоплен", "жкх", "труб", "авари", "котельн", "электр", "свет"), "C1"),
    (("обращен", "жалоб", "бюрократ", "очеред", "цифров", "сервис"), "C2"),
    (("смог", "воздух", "выброс", "дым", "экологи", "загрязн"), "E2"),
    (("парк", "сквер", "дерев", "озелен", "зелён", "зелен"), "E1"),
    (("пробк", "затор", "дорог", "трафик"), "T1"),
    (("автобус", "остановк", "транспорт", "лрт", "метро", "brt"), "T2"),
    (("школ", "детсад", "садик", "учени", "смен"), "S1"),
    (("поликлин", "больниц", "врач", "медиц", "здоров"), "S2"),
    (("преступ", "кража", "освещен", "камер", "безопасн улиц"), "B1"),
    (("дтп", "авари на дорог", "перехо", "сбил", "скорост"), "B2"),
]
_DISTRICTS = [("есил", "esil"), ("алмат", "almaty"), ("сарыарк", "saryarka"), ("байконур", "baikonur"), ("нур", "nura")]


def event_designer(text: str) -> Dict[str, Any]:
    low = text.lower()
    codes = [code for words, code in _KEYWORDS if any(w in low for w in words)]
    if not codes:
        codes = ["C1"]
    districts = [d for word, d in _DISTRICTS if word in low]
    if not districts:
        districts = ["saryarka", "nura"]  # чаще всего страдают самые слабые
    heavy = any(w in low for w in ("катастроф", "массов", "срыв", "коллапс", "чрезвычай"))
    delta = -14 if heavy else -8
    shocks = [{"district": d, "indicator": c, "delta": delta} for c in codes[:3] for d in districts[:2]]
    money = any(w in low for w in ("бюджет", "секвестр", "деньг", "финанс", "штраф", "подорожа"))
    return {
        "title": "Внеплановая ситуация в городе",
        "narrative": text.strip()[:300],
        "shocks": shocks[:6],
        "blocked_measures": [],
        "budget_delta": -10 if money else 0,
        "interpretation": "Ключ LLM не задан: показатели выбраны по ключевым словам в описании, масштаб шока взят типовым.",
        "_mode": "fallback",
    }
