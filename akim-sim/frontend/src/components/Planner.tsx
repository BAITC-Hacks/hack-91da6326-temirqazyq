    ...
    <div className="grid planner">
      <div>
        <section className="howto panel mb">
          ...
          <p className="muted">
            Выберите 5 мероприятий. Районные меры требуют района. Затем нажмите
          ...
          <div className="howto-steps" aria-label="Порядок действий">
            ...
            <span>
              <b>2</b> Рассчитать результат
            </span>
            <span>
              <b>3</b> Изучить совет AI
            </span>
          </div>
        </section>
        <div className="catalog-head row spread">
          <div>
            ...
            <p className="muted small">
              Нажмите «Добавить», чтобы включить меру в план.
            </p>
          </div>
        ...
        {visibleDirections.map(([dir, title]) => (
          <div className="dir-block" key={dir}>
            <h3>
              <i style={{ background: DIR_COLORS[dir] }} />
              {title}{' '}
              <span
                className="muted"
                style={{ textTransform: 'none', letterSpacing: 0 }}
              >
                · не более {meta.rules.max_per_direction}
              </span>
            </h3>
                    ...
                    >
                      ...
                      <div className="row small muted">
                        ...
                        <span>
                          ⏱ лаг {m.lag} кв. → эффект ×
                          {(
                            (meta.rules.horizon_quarters - m.lag) /
                            meta.rules.horizon_quarters
                          ).toFixed(2)}
                        </span>
                      </div>
                      <div className="fx">
                      ...
                      <div className="actions">
                        {isBlocked ? (
                          <span className="small error">
                            Заблокировано событием
                          </span>
                        ) : sel ? (
                          <>
                            <span className="small muted">
                              Выбрано: {districtName(meta, sel.district_id)}
                            </span>
                            <button
                              className="small"
                          ...
                          <>
                            {m.scope === 'district' && (
                              <select
                                value={pick[m.id] ?? ''}
                                onChange={(e) =>
                                  setPick({ ...pick, [m.id]: e.target.value })
                                }
                              >
                            ...
                            <button
                              ...
                              disabled={
                                full || (m.scope === 'district' && !pick[m.id])
                              }
                              onClick={() =>
                                add(
                                  m.id,
                                  m.scope === 'district' ? pick[m.id] : null,
                                )
                              }
                            >
                              Добавить
      ...
      <div className="panel sticky">
              ...
              >
                ...
                {d && m ? (
                  <>
                    ...
                    <button className="small ghost" onClick={() => remove(i)}>
                      ✕
                    </button>
                  </>
                ) : (
                  <span className="muted small">
                    Пустой слот — выберите мероприятие в каталоге
                  </span>
                )}
              </div>
            );
        ...
        <div className="mt row spread">
          <span className="small muted">Бюджет</span>
          <b className="num">
            {cost} / {budget}
          </b>
        </div>
        ...
        <div className="dircount mt">
          {Object.entries(meta.directions).map(([dir, title]) => {
            const n =
              ...
              decisions.filter(
                (d) =>
                  meta.measures.find((m) => m.id === d.measure_id)
                    ?.direction === dir,
              ).length;
            return (
              <div
        ...
        <div className="mt issues">
          {validation && validation.valid && (
            <div className="ok">Набор валиден — можно считать Score</div>
          )}
