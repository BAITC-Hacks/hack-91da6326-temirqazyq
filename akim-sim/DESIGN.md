...
> Этот раздел — надстройка проекта. При обновлении апстрима перезаписывается только часть выше.

## Почему ушли с linear.app

Первая версия интерфейса была собрана по `linear.app`. Она не сработала, и причина поучительная.

Лестница поверхностей Linear — `#0f1011 → #141516 → #18191a → #191a1b`. Замеренная светлота этих ступеней:
**0.0622 → 0.0818 → 0.0975**, то есть соседние слои различаются на 1.5–2%. На экране это неразличимо.
Слои перестали читаться как слои, и единственным инструментом разделения осталась линия. В результате на одном
экране планировщика оказалось **108 обведённых элементов из 337** — каждый третий в собственной рамке. Это и
даёт узнаваемый «сгенерированный дашборд»: одинаковые прямоугольники в одинаковых обводках, без иерархии.

Корень глубже: `DESIGN.md` линейки описывает **маркетинговую страницу** — огромная типографика, скриншоты
продукта, 80% пустоты, цвета почти нет. Наш продукт — плотный измерительный прибор: четырнадцать карточек мер,
таблица 5×10, шесть плиток, боковая навигация. Та же аскеза на такой плотности читается как серая каша.

Stripe решает ровно эту задачу: светлые поверхности, **тень как средство разделения** вместо обводки, и
достаточный контраст между `canvas`, `canvas-soft` и «утопленными» областями. После переезда рамок осталось
**11 из 337** — и все они там, где обводка что-то значит: поля ввода и вторичные кнопки.

## Что взято буквально

| Из stripe | Куда легло |
|---|---|
| `canvas` #ffffff, `canvas-soft` #f6f9fc | белые панели всплывают над мягким фоном страницы без единой рамки |
| Тени уровня 1 и 2 (`rgba(0,55,112,0.08)`) | `--lift-1`, `--lift-2`; третий уровень добавлен для модальных окон |
| `ink` #0d253d → `ink-secondary` → `ink-mute` | четыре уровня туши, все ≥4.5:1 на белом |
| Индиго `#533afd` | **только** primary-кнопки, фокус, активный шаг навигации, полоса бюджета |
| Дисплейные кегли весом 300 с отрицательным трекингом | заголовки и hero-число Score (48px, −0.96px) |
| Табличные цифры там, где важны числа | все таблицы, оси, дельты — `tabular-nums` |
| `rounded` 4/6/8/12/16/pill, `spacing` 2→64 | `--r-*`, `--s-*` |

Проприетарный Sohne заменён системным стеком с `SF Pro Display`, как предписывает сам файл.

Атмосферный градиентный меш из системы **не** используется: он рассчитан на верхнюю треть маркетинговой
страницы, а у нас там рабочая панель. Его роль — «поднимать» контент — выполняют тени.

## Отступление: слой данных

Stripe даёт один акцент. Измерительному прибору нужно кодировать идентичность (пять направлений), полярность
(вклад со знаком) и магнитуду (тепловая карта). Поэтому система расщеплена на два слоя:

* **Хром** — строго по stripe. Индиго не несёт данных **никогда**: он означает «сюда можно нажать».
* **Слой данных** — отдельный набор ролей, провалидированный скриптом под **белую** поверхность.

### Палитра направлений (categorical)

Палитра пересобрана при переезде: значения, отобранные для near-black фона Linear, на белом проваливают гейты.
Перебраны все назначения, отброшены не прошедшие, среди оставшихся выбрано самое осмысленное:

| Направление | Оттенок | Hex | Контраст на белом |
|---|---|---|---|
| Транспорт | blue | `#2a78d6` | 4.42:1 |
| Экология | aqua | `#1baf7a` | 2.82:1 |
| Соцсфера | violet | `#4a3aa7` | 8.56:1 |
| Безопасность | magenta | `#e87ba4` | 2.69:1 |
| Сервисы | yellow | `#eda100` | 2.17:1 |

`node scripts/validate_palette.js "#2a78d6,#1baf7a,#4a3aa7,#e87ba4,#eda100" --mode light --surface "#ffffff"`
→ полоса светлоты **PASS**, порог хромы **PASS**, смежная CVD-разделимость **ΔE 16.3** (цель ≥8),
порог нормального зрения **ΔE 19.6** (жёсткий порог ≥15).

Три оттенка из пяти дают на белом меньше 3:1. Это задокументированное послабление, но оно **обязывает**:
свотч никогда не стоит один — рядом всегда подпись направления, имя меры или значение. Свотч без текста
использовать нельзя.

Фиолетовый для соцсферы соседствует по тону с индиго-акцентом. Проверены все варианты палитры **без**
фиолетового — ни один не проходит гейты (лучший даёт CVD ΔE 6.1 при пороге 8). Поэтому фиолетовый оставлен,
а риск снят разной ролью: индиго живёт только на залитых кнопках с белым текстом, фиолетовый — на мелких
свотчах и барах рядом с подписью.

### Статусы и полярность

`good` `#0ca30c` · `warning` `#fab219` · `serious` `#ec835a` · `critical` `#d03b3b`, всегда со словом или знаком.

На белом фоне поправка обратная той, что была в тёмной теме: `#0ca30c` даёт лишь 3.35:1 — хватает для заливок
и полос, но не для текста. Положительные числа в тексте окрашены в `#006300` (7.54:1), роль называется
`--pos-ink`. Отрицательные берут `#d03b3b` (4.80:1) — на белом он проходит и для текста.

Вклад меры означает «помогло / навредило», поэтому это **статус**, а не идентичность: полюса `good` ↔ `critical`,
знак продублирован стороной от нулевой линии. «До → после» — одна синяя шкала двумя ступенями:
`#9ec5f4` (до, отступает) → `#2a78d6` (после). Тепловая карта — семантический heat со шкалой-легендой;
плотность тонировки снижена до 0.2, иначе тушь на светлом фоне перестаёт читаться.

## Формы графиков

Решения перенесены из прошлой версии без изменений: dumbbell вместо радара и вместо баров с ненулевой базой
(значение несёт положение точки, поэтому обрезанный диапазон законен); диверегирующие бары по знаку вместо
раскраски по направлению (идентичность там не декодировалась, а знак — то, зачем на график смотрят).
На Recharts осталась только гистограмма оракула; dumbbell, бары вклада и тепловая карта собраны руками.

## Ясность вместо украшений

Термины движка несут пунктирное подчёркивание и всплывающую расшифровку (`Hint` в `components/Viz.tsx`).
Коды показателей раскрываются там, где по ним принимают решение. Любая ячейка тепловой карты кликается и
показывает, из каких слагаемых сложилось число. Заблокированная кнопка объясняет, чего не хватает.
